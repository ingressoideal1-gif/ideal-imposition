...
with open('frontend/script.js', 'r', encoding='latin-1') as f:
    content = f.read()
...
content = content.replace(svg_listener.encode('utf-8').decode('latin-1'), pdf_listener.encode('utf-8').decode('latin-1'))
...
content = content.replace(svg_functions.encode('utf-8').decode('latin-1'), pdf_functions.encode('utf-8').decode('latin-1'))
...
content = content.replace(add_element_svg.encode('utf-8').decode('latin-1'), add_element_pdf.encode('utf-8').decode('latin-1'))
...
content = content.replace(type_label_old.encode('utf-8').decode('latin-1'), type_label_new.encode('utf-8').decode('latin-1'))
content = content.replace(type_badge_old.encode('utf-8').decode('latin-1'), type_badge_new.encode('utf-8').decode('latin-1'))
...
draw_pdf = draw_svg + ''' else if (el.type === 'PDF') {
                        ...
                        ctx.fillStyle = color;
                        ctx.textAlign = 'center';
                        ctx.fillText('PDF 100%', sz_w / 2, sz_h / 2 + (sz_h * 0.05));
                        ctx.textAlign = 'left';
                    }'''

content = content.replace(draw_svg.encode('utf-8').decode('latin-1'), draw_pdf.encode('utf-8').decode('latin-1'))

with open('frontend/script.js', 'w', encoding='latin-1') as f:
    f.write(content)
